...
namespace TestProject1
{
    ...
    public class LoginAcceptanceTest : TesteBase
    {
        ...
        [Test]
        public void testeUsuarioExistenteSenhaValida()
        {
        ...
        [Test]
        public void testeUsuarioExistenteSenhaInvalida()
        {
            int entradasEsperadasNaTabela = 1;
            ...
            driver.FindElement(By.Id("loginForm:username")).SendKeys("cfc");
            driver.FindElement(By.Id("loginForm:password")).SendKeys("c");
            driver.FindElement(By.Id("loginForm:submit")).Click();

            IWebElement element = driver.FindElement(By.XPath("id('messages')/li[1]"));

            bool acheiTextoLogadoComFalha = element.Text.Equals(texto_mensagem_logado_com_falha);
            Assert.IsTrue(acheiTextoLogadoComFalha, "Não encontrei elemento de login com falha");
